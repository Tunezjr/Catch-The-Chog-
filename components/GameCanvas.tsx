...
export default function GameCanvas() {
    ...
    function update(timestamp: number) {
      ...
      for (const f of g.fallers) {
        ...
        if (
          ...
          g.spawnInterval = Math.max(500, 1100 - g.score * 15);
          g.fallSpeed = Math.min(8, 2.2 + g.score * 0.12);
        } else if (f.y > GAME_H) {
  ...
  function shareOnX() {
    ...
    // window.location.origin is whatever domain the game is actually running
    // on (localhost while developing, your real domain once deployed) — so
    // this doesn't need to be hardcoded.
